...
//		list라는 ArrayList<Member> 타입 변수에 그 값을 저장하는 것이다.
	...
	public void selectByUserId(String userId) { // 멤버메뉴 case3에서 받아온게 어떤 자료형인지 명시해줘야하니까 ! 
		Member m =new MemberDao().selectByUserId(userId); // 여기서 바로 db로는 안되고 dao 호출 => dao로 이동해서 만들어주기 ! selectByUserID에 커서 올려보면 Member형
	...
	public void selectByUserName(String keyword) { // 메뉴에서 보내온 자료형 변수를 매개변수로 넣어두기
		ArrayList<Member> list =new MemberDao().selectByUserName(keyword); // 이게 있어야 디비로 => dao로 가자 => 하고 와서 커서 올려서 반환형 확인! 어레이리스트 제네릭 멤버형이다 => 그 자료형의 list에 저장해줘
